...
Lad os vælge 33 farver

```{r}
skuespil_farver <- qualitative_hcl(33, palette = "Dynamic")
```

Lad os se, hvordan de 33 farver placerer sig i et farverum
...
character_plot_with_color <- function(character_name, skuespil_farver, skuespil) {
  ...
  ggplot(aes(fill = title, y = freq, x = title)) + 
    ...
    facet_wrap(~year, ncol=nrow(skuespil), switch="x") +
