...
```{r}
my_file <- "test-data/Den_11_Junii_mod.jsonl"
...
my_play %>% 
  ...
  summarise(words = sum(n_spoken_words), act_number, scene_number) -> my_summary
...
```{r}
my_summary %>% 
  ggplot(aes(fill = speaker, y = words, x = scene_index, act_number, scene)) +
