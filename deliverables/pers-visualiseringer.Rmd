...
```{r}
# order the play titles
summary |> select(title)  |> distinct(title) |> arrange(desc(title)) |> pull(title) -> play_order

summary %>% 
  #filter(speaker %in% c("henrik","pernille","troels")) %>%
  filter(speaker %in%   c("henrik", "jeronimus", "leander")) %>%
  arrange(title) %>%
  ggplot(aes(fill = speaker, y = freq, x = factor(title, level = play_order))) +
    geom_bar(stat="identity") +
    theme(legend.position="bottom") +
    labs(
