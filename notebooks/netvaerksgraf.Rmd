...
```{r}
my_dir <-"test-data"
read_play_jsonl(here(my_dir, "Barselstuen_mod_nc.jsonl")) -> play
...
(distinct_speakers_Barselstuen %>%
  ...
  distinct(speaker) -> distinct_speakers_Barselstuen)
...
(distinct_speakers_Barselstuen %>%
    ...
    select(speaker) %>%
     distinct() %>% 
   rowid_to_column("id") -> nodes_Barselstuen) # add id column
