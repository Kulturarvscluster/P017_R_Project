...
For at gøre det nemme for os selv, starter vi med kun at se på Barselstuen
...
Vi er nu ved at være klar til at visualisere, hvem der siger hvor meget i Barselsstuen. Vi mangler bare at kombinere ovenstående kode
...
barselsstuen %>% 
  ...
  summarise(words = sum(n_spoken_words)) -> barselsstuen_vis
...
Først laver vi en liste over alle roller i Barselsstuen

```{r, rows.print=70}
print(barselsstuen_vis %>%
        ungroup() %>% 
  select(speaker) %>%
  distinct(speaker) %>%
