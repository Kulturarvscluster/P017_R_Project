...
These JSONL files can then be read into R using another utility function.
...
If we now look in the same folder, we see that we have gotten two .jsonl files
...
plays
```
Let's try to count how many rows each play has in the data. As stage remarks also result in individual rows, the count cannot be interpreted as the number of lines per play.
